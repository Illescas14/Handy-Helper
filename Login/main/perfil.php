...
function getUserAvailability($conn, $user_id)
{
...
$userProfiles = getUserProfiles($conn, $user_id);

// Obtener las reseñas del usuario
$userReviews = getUserReviews($conn, $user_id);

// Obtener la disponibilidad completa del usuario
$userAvailability = getUserAvailability($conn, $user_id);
    ...
    <main class="container profile-container">
            ...
            <div class="card mb-3" style="border-color: #48dfd0;">
                <div class="card-body">
                    <div class="profile-header">
                        <a href="dashboard.php" class="back-button" onclick="history.back()"><i class="fas fa-chevron-left"></i></a>
                        ...
                        <div class="ml-3">
                            ...
                            <a href="edit/editar_informacion_general.php"><i class="fas fa-pencil-alt"></i> Editar información</a>
                        </div>
                    </div>
                </div>
            </div>
                                ...
                                <?php foreach ($userProfiles as $index => $profile): ?>
                                    <?php if (!empty($profile['job_title'])): ?>
                                        <option value="<?php echo $profile['profile_id']; ?>" <?php if ($index == 0) echo 'selected'; ?>>
                                            <?php echo $profile['job_title']; ?></option>
                                    <?php endif; ?>
                                <?php endforeach; ?>
                            </select>
                        </div>
                                    ...
                                    <div class="card-body">
                                        <div class="info-card">
                                            <div class="card-section">
                                                ...
                                                <a href="edit/editar_profesion.php?profile_id=<?php echo $profile['profile_id']; ?>"><i class="fas fa-pencil-alt"></i></a>
                                            </div>
                                            <div class="card-section">
                                                <h3>Tareas y Precios</h3>
                                                <?php foreach ($profile['tasks'] as $task): ?>
                                                    <div class="task-item">
                                                        <p><strong><?php echo $task['task_name']; ?>:</strong> $<?php echo $task['price']; ?> MXN</p>
                                                        
                                                    </div>
                                                <?php endforeach; ?>
                                            </div>
                                        </div>
                                        <div class="card mt-3" style="border-color: #48dfd0;">
                                            <div class="card-header" style="background-color: #48dfd0; color: white;">
                                                <h3>Portafolio</h3>
                                                <a href="edit/editar_portafolio.php?profile_id=<?php echo $profile['profile_id']; ?>"><i class="fas fa-pencil-alt"></i> Editar Portafolio</a>
                                            </div>
                                            <div class="card-body portfolio">
                                                <div class="portfolio-images">
                                                    <?php foreach ($profile['portfolio_images'] as $image): ?>
                                                        <div class="portfolio-image">
                                                            <img src="../uploads/<?php echo $image['image_name']; ?>" alt="Imagen de portafolio" class="img-thumbnail" onclick="toggleImageSize(this)">
                                                        </div>
                                                    <?php endforeach; ?>
                                                </div>
                                            </div>
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-sm-4 text-center">
                            ...
                            <button type="button" class="btn" style="background-color: #48dfd0; border-color: #48dfd0;" data-toggle="modal" data-target="#reviewModal">Escribir reseña</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="modal fade" id="reviewModal" tabindex="-1" aria-labelledby="reviewModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header" style="background-color: #48dfd0; color: white;">
                    ...
                    <div class="modal-body">
                        <form id="review_form">
                            <div class="form-group">
                                ...
                                <select class="form-control" id="review_rating" style="border-color: #48dfd0;">
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control" id="review_text" rows="3" style="border-color: #48dfd0;"></textarea>
                            </div>
                            <button type="submit" class="btn" style="background-color: #48dfd0; border-color: #48dfd0;">Enviar</button>
                        </form>
                    </div>
                </div>
            </div>
        ...
        <script>
            $(document).ready(function () {
                ...
                // Eliminar el botón y modal de agregar reseña
                $("#reviewModal").remove();
                $("#review_form").remove();
            });
        </script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            var calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                dateClick: function (info) {
                    var status = confirm("¿Desea agregar disponibilidad para " + info.dateStr + "?");
                    if (status) {
                        $.ajax({
                            url: 'update_availability.php',
                            method: 'POST',
                            data: {
                                specific_date: info.dateStr,
                                status: 'available'
                            },
                ...
                eventClick: function (info) {
                    if (confirm("¿Desea eliminar esta fecha de disponibilidad?")) {
                        var dayOfWeek = getDayOfWeek(info.event.start.getDay());
                        $.ajax({
                            url: 'delete_availability.php',
                            method: 'POST',
                            data: {
                                id: info.event.id,
                                specific_date: info.event.startStr, // Cambiado a info.event.startStr para la fecha completa
